...
export function encodeInstructionArguments(
  ...
  const chunks = ix.arguments.reduce<ReadonlyUint8Array[]>(
    (chunks, ixArgumentNode) => {
      ...
      if (isIxArgumentOmitted(ixArgumentNode)) {
        // QUESTION(internal): better way to handle codec for each argument. preferrably with visitor
        //
        ...
        switch (ixArgumentNode.defaultValue.kind) {
          ...
          default: {
            throw new ArgumentError(
              `Not supported encoding for ${ixArgumentNode.name} argument of "${ixArgumentNode.type.kind}" kind`
            );
          }
        }
      } else if (
        ixArgumentNode.type.kind === "optionTypeNode" &&
        ...
        if (!input) {
          throw new ArgumentError(
            `Missing required argument: ${ixArgumentNode.name}`
          );
        }
...
export function validateArgumentsInput(
  root: RootNode,
  ...
  // console.log(getValidationItemsForTypeNode(ixNode));
...
function getRequiredIxArguments(ixNode: InstructionNode) {
  return ixNode.arguments.filter(
...
function validateOmittedArguments(
  ixNode: InstructionNode,
  argumentsInput: ArgumentsInput = {}
) {
