...
describe('Token 2022 Program: scaledUiAmount', () => {
    test('should initialize scaled UI amount mint extension [initializeScaledUiAmountMint]', async () => {
        ...
        const initScaledIx = await token2022Client.methods
            .initializeScaledUiAmountMint({ authority: payer, multiplier: 1.5 })
            .accounts({ mint })
            .instruction();
        ...
        expect(mintData.extensions).toMatchObject(
            some([{ __kind: 'ScaledUiAmountConfig', authority: payer, multiplier: 1.5 }]),
        );
    });

    test('should update multiplier for scaled UI amount [updateMultiplierScaledUiMint]', async () => {
        const ctx = new SvmTestContext({ defaultPrograms: true });
        const payer = ctx.createFundedAccount();
        const mint = ctx.createAccount();
